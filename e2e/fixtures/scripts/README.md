...
cd /Users/jingwuxu/Documents/Code/doctor-ai-agent-benchmark
  .venv/bin/python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli
```

**Claude CLI only (100 cases):**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --no-codex
```

**Codex only:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --no-claude
```

**Claude API (requires key):**
```bash
ANTHROPIC_API_KEY=sk-ant-... python e2e/fixtures/scripts/generate_llm_cases.py --no-codex
```

**Dry-run — print prompt without calling any LLM:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --dry-run --claude-cli --no-codex
```

**Custom output path:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --out /tmp/test_cases.json
```

**More cases per batch (default 10):**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --cases-per-batch 20
```

**More total data by repeating all theme batches 3 times:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --rounds 3
```

**Append 2 dedicated 神经/脑血管专科 batches:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --extra-neuro-specialty-batches 2
```

**Append all 10 神经/脑血管专科 batches:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --extra-neuro-specialty-batches 10
```

**Use only 神经/脑血管专科 batches:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --neuro-cerebro-only
```

**Use only the first 4 神经/脑血管专科 batches:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --neuro-cerebro-only --extra-neuro-specialty-batches 4
```

**Larger run example (10 batches × 20 cases × 3 rounds × 2 models = 1200 cases):**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --cases-per-batch 20 --rounds 3
```

**Larger neuro-focused run example:**
```bash
python e2e/fixtures/scripts/generate_llm_cases.py --claude-cli --cases-per-batch 20 --rounds 2 --extra-neuro-specialty-batches 7
```
