...
def test_admin_tables_human_input_e2e():
    ...
    # Keep label + prompt tables covered via user-facing APIs (no hidden SQL).
    ...
    patient_a_id = int(patient_a_row[0])
    ...
    try:
        ...
        patient_rows = _admin_rows("patients", doctor_id)
        assert len(patient_rows) >= 1
        assert any(_name_matches(row.get("patient_name"), patient_b) for row in _admin_rows("medical_records", doctor_id))
        assert any(int(row["id"]) == task_id for row in _admin_rows("doctor_tasks", doctor_id))
        assert any(_name_matches(row.get("patient_name"), patient_a) for row in _admin_rows("neuro_cases", doctor_id))
        ...
        assert any(int(row["label_id"]) == label_id for row in _admin_rows("patient_label_assignments", doctor_id))
        assert any(
        ...
        try:
            ...
            assert due_count >= 1
            assert (sent_count + failed_count) >= 1
        finally:
