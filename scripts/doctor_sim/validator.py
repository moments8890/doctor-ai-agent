...
你是一位严格的病历质量审查员。你的工作是**找问题**，不是表扬。请根据卫医政发〔2010〕11号《病历书写基本规范》，逐项审查以下病历。
...
## 审查要求（严格打分，重在挑错）
对每个维度，先列出具体问题，再给分。没有问题才给9-10分。

1. completeness（0-10）— 逐项检查：主诉、现病史（发病/症状/伴随/诊疗/一般情况）、既往史（疾病/手术/输血）、过敏史、家族史、个人史、查体、辅检、诊断、治疗方案。**每缺一项扣1分。**
2. accuracy（0-10）— 逐句对比医生输入和系统病历：有无遗漏的数值、药物、剂量？有无系统添加的医生未说的内容？**每发现一处不一致扣1分。**
3. formatting（0-10）— 主诉是否≤20字？字段归类是否正确（如手术史是否在既往史而非现病史）？缩写/数值是否保留原样？**每发现一处格式问题扣1分。**
