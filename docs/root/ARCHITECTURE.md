...
│   ├── seed_db.py            # CLI: export/import patients.db ↔ tests/fixtures/seed_data.json
...
- `WECHAT_NOTIFY_FALLBACK_TO_USER` (optional): fallback external_userid when
  doctor_id is not a valid WeCom customer user (errcode `40096`)
...
doctor_id (logical tenant key)
   ...
   ├─ doctor_knowledge_items (1:N by doctor_id)
...
- `id` (PK, autoincrement)
- `doctor_id` (indexed, required)
- `content` (required; doctor-authored reusable knowledge snippet)
- `created_at` (UTC timestamp)
- `updated_at` (UTC timestamp)
...
- knowledge fast-path:
  - `add_to_knowledge_base ...` / `添加知识库 ...` writes to `doctor_knowledge_items`;
  - successful clinical turns can auto-capture distilled snippets into the same table;
  - future routing calls load bounded knowledge snippets and inject them as a system context block.
...
Runtime config supports bounded doctor knowledge injection to avoid prompt overflow:

- `KNOWLEDGE_CANDIDATE_LIMIT`: max DB rows scanned per request before ranking.
- `KNOWLEDGE_MAX_ITEMS`: max selected snippets injected per request.
- `KNOWLEDGE_MAX_CHARS`: total character budget for injected knowledge block.
- `KNOWLEDGE_MAX_ITEM_CHARS`: per-snippet truncation budget before injection.
- `KNOWLEDGE_AUTO_LEARN_ENABLED`: enable/disable automatic knowledge capture.
- `KNOWLEDGE_AUTO_MAX_NEW_PER_TURN`: max auto-added snippets per successful clinical turn.
- `KNOWLEDGE_AUTO_MIN_TEXT_CHARS`: minimum input length for auto-learning consideration.
...
WECHAT_NOTIFY_FALLBACK_TO_USER=   # optional WeCom external_userid fallback
...
**Quality validation:** Re-run `bash scripts/test.sh chatlog-full` after model changes.
...
python scripts/chat.py http://host:8000  # custom host

# DB inspection
python scripts/db_inspect.py patients
python scripts/db_inspect.py records
python scripts/db_inspect.py record <id>

# DB seed (export / import)
python scripts/seed_db.py --export              # dump patients.db → tests/fixtures/seed_data.json
python scripts/seed_db.py --import             # load tests/fixtures/seed_data.json → patients.db
python scripts/seed_db.py --reset --import     # wipe then import (clean dev reset)
python scripts/seed_db.py --export --dry-run   # preview without writing

# DB UI
bash scripts/start_db_ui.sh              # → http://localhost:8001
...
│   ├── README.md
│   ├── TESTING.md
│   └── ARCHITECTURE.zh.patient-data-model.md
...
│   ├── db_inspect.py         # CLI: patients / records / record <id>
│   ├── seed_db.py            # CLI: export/import patients.db ↔ tests/fixtures/seed_data.json
│   └── run_chatlog_e2e.py    # Human-language E2E replay + DB assertions
│
├── e2e/fixtures/data/
│   ├── realworld_doctor_agent_chatlogs_e2e_v1.json
...
doctor_contexts
  doctor_id (PK)  — internal doctor key (can map from WeChat user id)
  summary (Text)  — LLM-compressed conversation (~120 chars)
  updated_at

doctor_session_states
  doctor_id (PK)
  current_patient_id (nullable FK→patients)
  pending_create_name (nullable)
  updated_at
...
doctor_notify_preferences
  ...
  schedule_type (immediate | interval | cron)
  interval_minutes | cron_expr | last_auto_run_at | updated_at
...
runtime_tokens
  token_key (PK)
  token_value | expires_at | updated_at

doctor_conversation_turns
  id (PK autoincrement)
  doctor_id (indexed) | role | content | created_at

doctors
  doctor_id (PK, internal stable key)
  name (nullable display name)
  channel (app | wechat)
  wechat_user_id (nullable)
  UNIQUE(channel, wechat_user_id)
  created_at | updated_at

patients
  id · doctor_id (indexed) · name · gender · year_of_birth · created_at
...
pytest e2e/integration/                      # auto-skipped if deps not running

# Human-language E2E replay (optional, expensive)
...
|-----|--------|------------|-------|
| Session history lost on restart (summary only persists) | Resolved | Implemented persisted turns table | Done |
...
| Single-process in-memory session | Medium | DB-backed rolling turns now in place; Redis optional for scale | 5 |
