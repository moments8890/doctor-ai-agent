...
| risk_score | Integer | 风险评分（risk-v2：CVD 患者优先取 neuro_cvd_context 中的量表评分；非 CVD 患者由关键词规则计算）|
...
存储字段：ICH Score（0–6）、Hunt-Hess（1–5）、Fisher/改良 Fisher、WFNS、动脉瘤位置/大小/颈宽/形态、Spetzler-Martin（1–5）、GCS（3–15）、PHASES Score（0–12）、血管痉挛状态、脑积水、烟雾病铃木分期、手术类型/日期/状态、mRS（0–6）、Barthel 指数。

`patient.risk_score` 的 risk-v2 规则引擎优先读取此表（ICH Score / Hunt-Hess / GCS / PHASES / mRS）。

**与 specialty_scores 的关系**：两表对同一病历双写。`neuro_cvd_context` 是完整临床上下文的查询主表；`specialty_scores` 是 per-score 的置信度/验证状态审计表，两者角色互补。如两表同一字段值不一致，以 `neuro_cvd_context` 为准。
