# Intake Pipeline Extensibility — Phase 0 (Skeleton + Plumbing)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Lay the DB and type foundation for the polymorphic intake pipeline — `template_id` column on `intake_sessions`, `preferred_template_id` on `doctors`, a new `form_responses` table, and retirement of the dead `draft_created` status. **Zero user-visible behavior change.** Every session continues to run under `medical_general_v1`.

**Architecture:** Additive DB migration (new columns + new table + data backfill). ORM + dataclass + CRUD thread `template_id` through create / load / save. `prompt_composer` accepts an ignored `template_id` passthrough so Phase 1 can wire it for real without another signature churn. An empty `src/domain/intake/` package gets created and stays empty until Phase 1.
...
**Reference:** Spec `docs/superpowers/specs/2026-04-22-intake-pipeline-extensibility-design.md` §§ 4a, 5a, 6a (Phase 0 row).
...
Create:
- `alembic/versions/c9f8d2e14a20_intake_template_id.py` — migration
- `src/db/models/form_response.py` — `FormResponseDB` ORM model
- `src/domain/intake/__init__.py` — empty package placeholder
- `tests/db/test_migration_intake_template_id.py` — forward + round-trip test
- `tests/core/test_form_response_model.py` — FormResponseDB smoke test
- `tests/core/test_intake_session_template_id.py` — dataclass / CRUD round-trip

Modify:
- `src/db/models/intake_session.py` — add `template_id`; drop `draft_created` from `IntakeStatus`
- `src/db/models/doctor.py` — add `preferred_template_id`
- `src/domain/patients/intake_session.py` — dataclass field + CRUD threading
- `src/agent/prompt_composer.py` — add `template_id` kwarg (ignored passthrough) to `compose_for_doctor_intake` and `compose_for_patient_intake`
- `src/channels/web/doctor_intake/turn.py` — accept `template_id` form field; forward into `create_session`. (Patient endpoint unchanged — see Task 7 scope note.)
- `src/channels/web/doctor_dashboard/admin_overview.py:145` — `_completed_statuses = ("confirmed",)`
- `tests/core/test_intake_session_mode.py:31-33` — flip assertion (status is gone, not present)
...
**Files:**
- Create: `alembic/versions/c9f8d2e14a20_intake_template_id.py`
- Create: `tests/db/test_migration_intake_template_id.py`

- [ ] **Step 1: Write the failing migration round-trip test**

`tests/db/test_migration_intake_template_id.py`:
...
/Volumes/ORICO/Code/doctor-ai-agent/.venv/bin/python -m pytest \
    tests/db/test_migration_intake_template_id.py -v \
    --rootdir=/Volumes/ORICO/Code/doctor-ai-agent
```
...
`alembic/versions/c9f8d2e14a20_intake_template_id.py`:

```python
"""Intake template_id + form_responses + retire draft_created.

Phase 0 of the intake-pipeline-extensibility work. See the spec at
docs/superpowers/specs/2026-04-22-intake-pipeline-extensibility-design.md
...
def upgrade() -> None:
    # 1. intake_sessions.template_id — server_default fills existing rows,
    ...
    op.create_index(
        "ix_intake_template", "intake_sessions", ["template_id"],
    )
    ...
    op.execute(
        "UPDATE intake_sessions SET status='confirmed' "
...
.venv/bin/python -c "from sqlalchemy import create_engine, inspect; e = create_engine('sqlite:///local.db'); i = inspect(e); print('template_id' in {c[\"name\"] for c in i.get_columns('intake_sessions')}); print('form_responses' in i.get_table_names())"
```
...
```
git add alembic/versions/c9f8d2e14a20_intake_template_id.py \
        tests/db/test_migration_intake_template_id.py
...
## Task 2: ORM — `IntakeSessionDB.template_id` + drop `draft_created` from enum

**Files:**
- Modify: `src/db/models/intake_session.py`
- Modify: `tests/core/test_intake_session_mode.py:31-33`

- [ ] **Step 1: Flip the stale assertion**

Existing `tests/core/test_intake_session_mode.py:31-33`:

```python
def test_intake_status_has_draft_created():
    assert hasattr(IntakeStatus, "draft_created")
    assert IntakeStatus.draft_created == "draft_created"
```
...
```
/Volumes/ORICO/Code/doctor-ai-agent/.venv/bin/python -m pytest \
    tests/core/test_intake_session_mode.py::test_intake_status_does_not_have_draft_created -v \
    --rootdir=/Volumes/ORICO/Code/doctor-ai-agent
```
...
In `src/db/models/intake_session.py`, change the `IntakeStatus` enum (currently at lines 15-22) to:
...
Then in the `IntakeSessionDB` class body (currently lines 25-41), add `template_id` between `mode` and `collected`:
...
```
/Volumes/ORICO/Code/doctor-ai-agent/.venv/bin/python -m pytest \
    tests/core/test_intake_session_mode.py -v \
    --rootdir=/Volumes/ORICO/Code/doctor-ai-agent
```
...
```
git add src/db/models/intake_session.py tests/core/test_intake_session_mode.py
git commit -m "refactor(db): add IntakeSessionDB.template_id + drop draft_created enum"
```
...
- Create (append to): `tests/core/test_intake_session_template_id.py`

- [ ] **Step 1: Write a failing test**

`tests/core/test_intake_session_template_id.py` (new file — Task 5 appends more to it):
...
The `fresh_db` fixture already exists in `tests/conftest.py` — it runs Alembic head against an in-memory SQLite. Confirm with `grep -n "fresh_db" tests/conftest.py`. If it does not exist, add this to `tests/core/test_intake_session_template_id.py` above the tests:
...
```
/Volumes/ORICO/Code/doctor-ai-agent/.venv/bin/python -m pytest \
    tests/core/test_intake_session_template_id.py -v \
    --rootdir=/Volumes/ORICO/Code/doctor-ai-agent
```
...
```
git add src/db/models/doctor.py tests/core/test_intake_session_template_id.py
...
## Task 5: Dataclass + CRUD — `IntakeSession.template_id`

**Files:**
- Modify: `src/domain/patients/intake_session.py`
- Modify: `tests/core/test_intake_session_template_id.py` (append)

- [ ] **Step 1: Append failing tests for CRUD threading**

In `tests/core/test_intake_session_template_id.py`, append:
...
Expected: three failures, all `TypeError: create_session() got an unexpected keyword argument 'template_id'` or `AttributeError: 'IntakeSession' object has no attribute 'template_id'`.

- [ ] **Step 3: Thread `template_id` through the dataclass and CRUD**

In `src/domain/patients/intake_session.py`:
...
Inside `create_session`, update the `IntakeSessionDB(...)` construction (around line 73-85) to pass `template_id=template_id`, and update the returned dataclass construction (around line 90-97) to include `template_id=template_id`.

Update `load_session` (around lines 100-122) — add `template_id=row.template_id` to the returned `IntakeSession(...)`.

Update `save_session` (around lines 125-146) — add `row.template_id = session.template_id` to the field updates.

Update `get_active_session` (around lines 149-175) — add `template_id=row.template_id` to the returned `IntakeSession(...)`.
...
```
git add src/domain/patients/intake_session.py \
        tests/core/test_intake_session_template_id.py
git commit -m "refactor(intake): thread template_id through dataclass + CRUD"
```
...
Append to `tests/core/test_intake_session_template_id.py`:
...
```
/Volumes/ORICO/Code/doctor-ai-agent/.venv/bin/python -m pytest \
    tests/core/test_intake_session_template_id.py::test_prompt_composer_accepts_template_id_kwarg -v \
    --rootdir=/Volumes/ORICO/Code/doctor-ai-agent
```
...
- `compose_for_doctor_intake` (around line 198) — add `template_id: str = "medical_general_v1"` as a keyword-only parameter, after the existing trailing params. Do not reference it inside the function body yet. Add a one-line docstring note: `# Accepted for Phase-1 plumbing; currently ignored.`
- `compose_for_patient_intake` (around line 238) — same change.
...
```
git add src/agent/prompt_composer.py tests/core/test_intake_session_template_id.py
git commit -m "refactor(prompt): accept template_id kwarg in intake composers"
```
...
**Files:**
- Modify: `src/channels/web/doctor_intake/turn.py` (endpoint around line 60-85, `_first_turn` around line 88-107)

**Scope note:** only the doctor endpoint changes. The patient endpoint (`/patient/intake/start` at `patient_intake_routes.py:49-90`) takes no request body — patients don't choose their template; the doctor's `preferred_template_id` would, but that fallback lands in Phase 1. For Phase 0, the patient endpoint keeps calling `create_session(...)` with no `template_id` argument, so `IntakeSession.template_id` defaults to `"medical_general_v1"` (Task 5). Nothing else to wire on the patient side.
...
```
sed -n '58,115p' src/channels/web/doctor_intake/turn.py
```

You'll see:
- `intake_turn_endpoint` route handler with `text`, `session_id`, `doctor_id`, `patient_id`, `file`, `authorization` as `Form`/`File`/`Header` params.
...
In `src/channels/web/doctor_intake/turn.py`, update `intake_turn_endpoint` to accept one more form field (insert it between `patient_id` and `file` to keep positional arg clustering readable):
...
First, grep for an existing doctor-intake endpoint test to copy its auth setup:

```
ls tests/channels/ 2>/dev/null
grep -rln "doctor_intake\|doctor/intake/turn" tests/ --include="*.py"
```

If a harness exists (likely in `tests/channels/` or `tests/api/`), add a test in the same style:

```python
# tests/channels/test_doctor_intake_template_id.py
...
If the project has no HTTP-level test harness for doctor_intake, add a thinner unit test against `_first_turn` instead, mocking `create_session`:
...
async def test_first_turn_passes_template_id_to_create_session():
    with patch(
        "channels.web.doctor_intake.turn.create_session",
        new=AsyncMock(),
    ) as mock_create:
        mock_create.return_value = type("S", (), {"id": "s1", "collected": {}})()
        with patch(
            "channels.web.doctor_intake.turn.intake_turn",
            new=AsyncMock(),
        ), patch(
            "channels.web.doctor_intake.turn.resolve",
            new=AsyncMock(),
        ):
...
git add src/channels/web/doctor_intake/turn.py \
        tests/channels/test_doctor_intake_template_id.py  # or tests/core/test_doctor_first_turn_template_id.py
git commit -m "feat(api): accept optional template_id on doctor intake /turn"
```
...
- [ ] **Step 4: Run any admin_overview test + the full intake suite**
...
## Task 9: Empty `src/domain/intake/` package

**Files:**
- Create: `src/domain/intake/__init__.py`

- [ ] **Step 1: Verify the directory does not exist yet**

```
ls src/domain/intake 2>/dev/null || echo "does not exist — good"
```

- [ ] **Step 2: Create the package stub**

`src/domain/intake/__init__.py`:
...
```
grep -rn "from domain.intake\|import domain.intake" src tests
```
...
```
git add src/domain/intake/__init__.py
git commit -m "chore(intake): reserve domain/intake package for Phase 1"
```
...
- [ ] **Step 2: Run the intake-specific suite in full verbose**
...
grep -rn "IntakeSession\b" src --include="*.py"
# Expected: every result site either has template_id or only touches the
# dataclass id/doctor_id — eyeball-check.

# domain/intake is still empty apart from __init__.py
ls src/domain/intake/
# Expected: __init__.py only.
```

- [ ] **Step 4: Run the intake sim baseline (if available)**
...
Run whatever `reply_sim` / `intake_sim` script the repo exposes, against `main` HEAD and against the current HEAD, and diff pass rates. Phase 0 target: **delta ≤ 2%**, per spec §7a.
...
- [ ] `intake_sessions.template_id` exists, NOT NULL, indexed
...
- [ ] No row in `intake_sessions` has status `draft_created`
- [ ] `IntakeStatus` enum has exactly 4 values: active, reviewing, confirmed, abandoned
- [ ] `IntakeSession` dataclass carries `template_id`; `create_session` / `load_session` / `save_session` / `get_active_session` all thread it
- [ ] `compose_for_doctor_intake` / `compose_for_patient_intake` accept `template_id` (ignored)
...
- [ ] `src/domain/intake/__init__.py` exists and is empty
...
- Move any medical logic out of `intake_models.py` / `completeness.py` / `doctor_intake/shared.py`.
