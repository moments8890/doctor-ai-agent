...
Status as of 2026-03-29. Tracks which intake pipeline scenarios have E2E sim coverage.
...
| A4 | Doctor KB loaded — intake shaped by rules | — | KB rules | patient | ❌ Not tested |
...
- **A4**: `load_knowledge()` injects doctor KB (L4 Doctor Rules). Tests whether doctor-specific rules shape the intake. Example: KB says "动脉瘤患者必须问是否服用抗凝药" → AI should ask about anticoagulants. Needs: seed `doctor_knowledge_items` rows.
- **A5**: Both A2 and A4 active simultaneously. Tests interaction between prior records and KB rules.
- **A6**: Doctor-mode intake uses `create_record` intent with 14 fields (adds physical exam, diagnosis, treatment plan). Entirely different completeness gate.

## B. Intake Quality

| # | Scenario | Status |
|---|----------|:---:|
| B1 | Standard comorbidity elicitation (HTN/DM/surgery/anticoagulants) | ✅ Fixed — checklist in intake prompt |
...
- **B2**: When patient says "有高血压" without details, AI should follow up: "持续多久？吃什么药？控制得怎么样？" Rule 15 in intake prompt covers this but no dedicated persona tests it.
...
| # | Scenario | Status |
|---|----------|:---:|
| F1 | Pending-review history leakage into new intake | ❌ Not tested |
...
| # | Scenario | Status |
|---|----------|:---:|
| H1 | LLM outage mid-intake → "系统繁忙" → patient retries | ❌ Not tested |
