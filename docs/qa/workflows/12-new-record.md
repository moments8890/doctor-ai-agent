...
**Area:** `src/pages/doctor/IntakePage.jsx` (doctor-side intake),
...
`components/doctor/IntakeCompleteDialog.jsx`, `components/ImportChoiceDialog.jsx`,
API: `doctorIntakeTurn`, `doctorIntakeConfirm`, `triggerDiagnosis`
...
- `IntakeCompleteDialog` at end: shows all collected fields; confirm
...
- Cancel: exits intake without saving, returns to patient list.
...
- Desktop layout variant (IntakePage renders in the detail pane on
...
| # | Action | Verify |
|---|--------|--------|
| 1.1 | Navigate to `/doctor/my-ai` → tap "新建病历" | Navigates to `/doctor/patients/new`; IntakePage renders with welcome message: "病历采集模式已开启..." |
| 1.2 | SubpageHeader visible | Back arrow `‹`, title area shows intake/record context |
...
| 2.4 | Multiple turns until status becomes `reviewing` or fields are collected | Progress bar reaches or nears 100%; AI signals completion or `IntakeCompleteDialog` opens |

### 3. Field review + confirm

| # | Action | Verify |
|---|--------|--------|
| 3.1 | `IntakeCompleteDialog` opens (or FieldReviewCards appear inline) | All collected NHC fields displayed: 主诉, 现病史, 既往史, etc. |
...
| 3.4 | Post-confirm state | IntakePage exits; navigates back to patient list or patient detail; patient's record list now shows a new record with status `待审核` and source `门诊` or `口述` |
...
| # | Action | Verify |
|---|--------|--------|
| 5.1 | Mid-intake, tap back arrow `‹` | Confirm dialog "放弃当前采集？" (or similar) with 取消 / 确认 |
| 5.2 | Confirm abandon | Intake exits; no record created; returns to patient list |
| 5.3 | Cancel the abandon dialog | Stays in intake; no data lost |
...
- **Session resume** — if `resumeSessionId` is passed (from chat intake),
...
spec — it relied on the hero-path-qa-plan.md §7 patient-portal intake
...
- **Welcome message is blank** — `patientContext` may be undefined;
  IntakePage.jsx:48 falls back to generic greeting. If the generic
  doesn't render, check the useEffect on line 72.
- **AI never replies** — `doctorIntakeTurn` API call failed silently.
  Check the backend intake session table and `NO_PROXY=*` env var.
...
- **Confirm button does nothing** — `doctorIntakeConfirm` may fail silently;
  check the API response and the error handler in IntakePage.
