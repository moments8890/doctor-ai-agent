...
### 6. `structuring.consultation_suffix` — 已移除

**状态：** 已合并到 `structuring` 基础提示词的「问诊对话」章节。

---

### 7. `structuring.followup_suffix` — 已移除

**状态：** 已合并到 `structuring` 基础提示词的「复诊/随访」章节。`detect_followup_from_text()` 及关键词列表已删除。
