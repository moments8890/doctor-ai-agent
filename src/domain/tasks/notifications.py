...
# Dependency-inverted send function — registered at startup by main.py.
# services/ never imports from channels/ directly.
_send_fn = None


def register_sender(fn):
    """Register the channel-specific message sender (called from main.py)."""
    global _send_fn
    _send_fn = fn
...
async def _send_via_wechat(doctor_id: str, message: str) -> None:
    ...
    try:
        if _send_fn is None:
            raise RuntimeError("No message sender registered — call register_sender() at startup")
        await _send_fn(target_user, message)
        ...
        if _is_invalid_wechat_user_error(e) and fallback_to_user and fallback_to_user != target_user:
            ...
            await _send_fn(fallback_to_user, message)
            return
        raise
