...
async def get_review_detail_endpoint(queue_id: int, doctor_id: str = Query(...)):
...
async def confirm_review_endpoint(queue_id: int, doctor_id: str = Query(...)):
    raise HTTPException(status_code=501, detail="ReviewQueue table removed")


@router.patch("/api/manage/review-queue/{queue_id}/record", include_in_schema=True)
async def update_review_field_endpoint(queue_id: int, doctor_id: str = Query(...)):
    raise HTTPException(status_code=501, detail="ReviewQueue table removed")
