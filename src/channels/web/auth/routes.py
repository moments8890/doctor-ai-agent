...
async def auth_me(
    ...
    row = (
        await db.execute(select(Doctor).where(Doctor.doctor_id == str(doctor_id)).limit(1))
    ).scalar_one_or_none()
    if row:
...
from .miniapp import router as _miniapp_router  # noqa: E402
from .invite import router as _invite_router    # noqa: E402
