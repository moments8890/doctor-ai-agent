...
function SectionHeader({ children }) {
  return (
    <Typography sx={{ fontSize: TYPE.heading.fontSize, fontWeight: 600, color: "#333", mb: 1 }}>
      {children}
    </Typography>
  );
}

const CONFIDENCE_COLORS = { 高: "#07C160", 中: "#ff9500", 低: "#999" };
const URGENCY_COLORS = { 急诊: "#FA5151", 紧急: "#e65100", 常规: "#07C160" };
const INTERVENTION_COLORS = { 手术: "#FA5151", 转诊: "#e65100", 药物: "#07C160", 观察: "#999" };
        ...
        {caseReferences.map((ref, i) => (
          ...
          >
            ...
            <Stack direction="row" alignItems="center" flexWrap="wrap" gap={0.5}>
              ...
              <Typography
                component="span"
                sx={{ fontSize: TYPE.secondary.fontSize, fontWeight: 600, color: "#333" }}
              >
                {ref.chief_complaint}
              </Typography>
              {ref.final_diagnosis && (
                <>
                  <Typography
                    component="span"
                    sx={{ fontSize: TYPE.secondary.fontSize, color: "#999" }}
                  >
                    →
                  </Typography>
                  <Typography
                    component="span"
                    sx={{ fontSize: TYPE.secondary.fontSize, fontWeight: 600, color: "#07C160" }}
                  >
                    {ref.final_diagnosis}
                  </Typography>
                </>
            ...
            <Stack direction="row" spacing={2} sx={{ mt: 0.6 }} flexWrap="wrap">
              {ref.treatment && (
                <Typography sx={{ fontSize: TYPE.caption.fontSize, color: "#555" }}>
                  治疗: {ref.treatment}
                </Typography>
              )}
              {ref.outcome && (
      ...
      <Stack spacing={0.5}>
        {redFlags.map((flag, i) => (
          <Stack key={i} direction="row" spacing={0.8} alignItems="flex-start">
            <Typography sx={{ fontSize: TYPE.secondary.fontSize, color: "#e65100", flexShrink: 0 }}>
              •
            </Typography>
            <Typography sx={{ fontSize: TYPE.secondary.fontSize, color: "#e65100", lineHeight: 1.6 }}>
              {flag}
            </Typography>
          </Stack>
        ))}
            ...
            <Stack direction="row" alignItems="flex-start">
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Stack direction="row" alignItems="center" flexWrap="wrap">
                  ...
                  {item.confidence && (
                    <StatusBadge label={item.confidence} colorMap={CONFIDENCE_COLORS} />
                  )}
                </Stack>
                {item.reasoning && (
                  <Typography
                    sx={{ fontSize: TYPE.caption.fontSize, color: "#555", mt: 0.4, lineHeight: 1.6 }}
                  >
                    {item.reasoning}
                  </Typography>
                )}
            ...
            <Stack direction="row" alignItems="flex-start">
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Stack direction="row" alignItems="center" flexWrap="wrap">
                  ...
                  {item.urgency && <StatusBadge label={item.urgency} colorMap={URGENCY_COLORS} />}
                </Stack>
                {item.rationale && (
                  <Typography
                    sx={{ fontSize: TYPE.caption.fontSize, color: "#555", mt: 0.4, lineHeight: 1.6 }}
            ...
            <Stack direction="row" alignItems="flex-start">
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Stack direction="row" alignItems="center" flexWrap="wrap">
                  ...
                  {item.intervention && (
                    <StatusBadge label={item.intervention} colorMap={INTERVENTION_COLORS} fallbackColor="#666" />
                  )}
                </Stack>
                {item.description && (
                  <Typography
  ...
  return (
    <Box>
      ...
      <Box sx={{ bgcolor: "#fff", borderRadius: 2, p: "12px 16px", mb: 1,
        borderLeft: "3px solid #07C160" }}>
        <Typography sx={{ fontSize: TYPE.action.fontSize, fontWeight: 700, color: "#333" }}>
          AI 诊断建议
        </Typography>
      </Box>
      ...
      {/* 6. Disclaimer */}
      <Box sx={{ px: 1, pb: 1 }}>
        <Typography sx={{ fontSize: TYPE.micro.fontSize, color: "#bbb", textAlign: "center" }}>
          AI建议仅供参考，最终诊断由医生决定
        </Typography>
      </Box>
    </Box>
