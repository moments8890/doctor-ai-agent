        ...
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 1, mb: 0.3 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 0.8, flexWrap: "wrap" }}>
            ...
            {(Array.isArray(current.tags) ? current.tags : []).map((tag, i) => (
              <Typography key={i} sx={{ fontSize: 11, color: "#999", bgcolor: "#f5f5f5", px: 0.6, borderRadius: 0.5 }}>
                {tag}
              </Typography>
            ))}
          </Box>
  ...
  return (
    <Box sx={{ px: 2, pb: 1.5, pt: 0 }}>
      ...
      <Box sx={{ bgcolor: "#f9f9f9", borderRadius: 1.5, p: 1.5 }}>
        <Typography variant="body2" sx={{ whiteSpace: "pre-wrap", fontSize: 13, color: "#333" }}>{current.content || "（无记录内容）"}</Typography>
      </Box>
    </Box>
  );
}
