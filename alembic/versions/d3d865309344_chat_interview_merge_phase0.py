...
def upgrade() -> None:
    ...
    op.add_column("medical_records", sa.Column("red_flag", sa.Boolean, nullable=False, server_default=sa.text("0")))
    op.add_column("medical_records", sa.Column("intake_segment_id", sa.String(64), nullable=True))
    op.add_column("medical_records", sa.Column("dedup_skipped_by_patient", sa.Boolean, nullable=False, server_default=sa.text("0")))
    op.create_index("ix_medical_records_intake_segment_id", "medical_records", ["intake_segment_id"])

    # DoctorKnowledgeItem additions
    op.add_column("doctor_knowledge_items", sa.Column("patient_safe", sa.Boolean, nullable=False, server_default=sa.text("0")))

    # Doctor additions
    op.add_column("doctors", sa.Column("kb_curation_onboarding_done", sa.Boolean, nullable=False, server_default=sa.text("0")))

    # PatientMessage additions
    op.add_column("patient_messages", sa.Column("retracted", sa.Boolean, nullable=False, server_default=sa.text("0")))
