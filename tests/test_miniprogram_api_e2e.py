"""小程序 E2E 集成测试：模拟真实多轮对话，验证病历创建和查询完整链路。"""
...
def _make_scripted_turns() -> list[dict]:
    return [
        ...
        {"user": "初步考虑不稳定型心绞痛，继续监测肌钙蛋白。", "reply": "已更新诊断与后续检查计划。", "keyword": "诊断"},
        {"user": "给他安排三天后复诊提醒。", "reply": "已创建随访任务：3天后复诊。", "keyword": "随访任务"},
        {"user": "查看他的最近病历。", "reply": "已返回患者【张建国】最近病历摘要。", "keyword": "最近病历"},
        {"user": "再查一下我当前待办任务。", "reply": "当前有1条待办任务：张建国复诊提醒。", "keyword": "待办任务"},
        {"user": "好的，今天先这样，谢谢。", "reply": "不客气，需要时随时继续补录病历。", "keyword": "随时"},
    ]


def _build_scripted_chat_mock(scripted_turns: list[dict], state: dict, doctor_id: str):
    async def _fake_records_chat(chat_input, doctor_id=doctor_id):
        idx = state["turn"]
        expected = scripted_turns[idx]
        assert doctor_id == f"wxmini_openid_mini_e2e_chat10"
        assert chat_input.doctor_id == f"wxmini_openid_mini_e2e_chat10"
    ...
    return _fake_records_chat


async def _run_scripted_chat_session(client, scripted_turns: list[dict], token: str):
    headers = {"Authorization": f"Bearer {token}"}
    history = []
...
async def test_miniprogram_real_life_10_turn_chat_e2e(session_factory):
    ...
    scripted_turns = _make_scripted_turns()
    state = {"turn": 0}
    fake_chat = _build_scripted_chat_mock(scripted_turns, state, "wxmini_openid_mini_e2e_chat10")
    ...
    with patch("routers.auth.AsyncSessionLocal", session_factory), patch.dict(
        ...
        async with AsyncClient(transport=transport, base_url="http://testserver") as client:
            ...
            await _run_scripted_chat_session(client, scripted_turns, token)
...
async def _run_live_chat_turns(client, turns: list[str], headers: dict, expected_doctor_id: str):
    history = []
    for idx, text in enumerate(turns):
        ...
        assert me_resp.status_code == 200
        assert me_resp.json()["doctor_id"] == expected_doctor_id
...
async def test_miniprogram_real_life_10_turn_chat_live_records_path_e2e(session_factory):
    ...
    with patch("routers.auth.AsyncSessionLocal", session_factory), patch(
        ...
        async with AsyncClient(transport=transport, base_url="http://testserver") as client:
            ...
            await _run_live_chat_turns(client, _LIVE10_TURNS, headers, "wxmini_openid_mini_e2e_live10")
