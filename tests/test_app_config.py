...
def test_load_env_prefers_shared_file():
    shared = Path("/tmp/shared.env")
    with patch.object(Path, "exists", return_value=True), patch("utils.app_config.load_dotenv") as mocked:
        source = load_env_from_shared_or_local(shared)

    assert source == shared
    mocked.assert_called_once_with(dotenv_path=shared)


def test_load_env_falls_back_to_local_dotenv():
    shared = Path("/tmp/shared.env")
    with patch.object(Path, "exists", return_value=False), patch("utils.app_config.load_dotenv") as mocked:
        source = load_env_from_shared_or_local(shared)

    assert source == Path(".env")
