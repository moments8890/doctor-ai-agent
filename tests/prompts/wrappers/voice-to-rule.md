...
- `custom` — anything else (patient communication style, operational rules)
...
Output:
```json
{"content": "前交通动脉瘤术后第二周关注患者记忆变化，复诊时询问近期记忆清晰度", "category": "followup", "error": null}
```
...
Output:
```json
{"content": "服用抗凝药的患者出现新发头痛或头痛加重时，警惕脑出血，建议立即影像学检查", "category": "diagnosis", "error": null}
```
...
Output:
```json
{"content": "阿托伐他汀 20mg 晚上睡前服用", "category": "medication", "error": null}
```
...
Output:
```json
{"content": null, "category": null, "error": "no_rule_found"}
```
...
Output:
```json
{"content": null, "category": null, "error": "multi_rule_detected"}
```
