"""
Patient intake session ownership: /turn must reject requests from a patient
...
def _make_session(session_id: str, patient_id: int):
    """Create a mock IntakeSession with the given patient_id."""
...
async def test_turn_rejects_wrong_patient():
    ...
    from channels.web.patient_intake_routes import turn, IntakeTurnRequest

    owner_patient_id = 1
    attacker_patient_id = 2
    session_id = "session-uuid-abc"

    body = IntakeTurnRequest(session_id=session_id, text="你好")
    ...
    with patch(
        ...
        "channels.web.patient_intake_routes.load_session",
        new_callable=AsyncMock,
...
async def test_turn_allows_owner():
    ...
    body = IntakeTurnRequest(session_id=session_id, text="我头痛")
    ...
    with patch(
        ...
        "channels.web.patient_intake_routes._get_engine",
...
async def test_turn_returns_404_for_nonexistent_session():
    ...
    body = IntakeTurnRequest(session_id="does-not-exist", text="你好")
