"""Unit tests for tools/seed_db.py — all I/O (sqlite3, file) is mocked."""
...
import tools.seed_db as seed_db
from tools.seed_db import (
    export_fixture,
    import_fixture,
    main,
    reset_tables,
...
def _fake_fixture_path(content: str = "", exists: bool = True) -> MagicMock:
    ...
    p.parent = MagicMock()
    p.__str__ = lambda self: "/fake/dev/seed_data.json"
    return p
...
class TestImportFixture:
    ...
    @patch("tools.seed_db._ensure_schema")
    @patch("tools.seed_db._connect")
...
class TestMain:
    ...
    @patch("tools.seed_db.export_fixture")
