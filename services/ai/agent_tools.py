...
_SYSTEM_PROMPT = (
    ...
    "即使消息含有「创建」「新患者」等建档信号，只要有临床内容就用 add_medical_record（系统会自动建档）。\n"
    ...
    "▌其他意图\n"
    "- 仅介绍患者身份（无临床内容、无提醒、无更正）→ create_patient\n"
...
_SYSTEM_PROMPT_COMPACT = (
    ...
    "临床信息->add_medical_record（即使含创建信号，有临床内容就用add_medical_record，系统自动建档）；仅创建且无临床内容->create_patient；"
